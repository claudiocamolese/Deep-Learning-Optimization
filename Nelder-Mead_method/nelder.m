function [xk, fk, execution_time, k] = nelder(f, x, n, rho, chi, gamma, sigma)
    ...
    % Only for rosenbrock function: plot 2D
    if n==2
        figure;
        hold on;
        fcontour(@(x, y) (1 - x).^2 + 100 * (y - x.^2).^2, [-2, 2, -1, 3], 'LineWidth', 0.6, 'LevelList', logspace(0, 3, 9));
        colormap parula;
        colorbar;
    ...
    while k <= max_iter && max(vecnorm(simplex - mean(simplex, 2), 2, 1)) >= tol
        ...
        % Store best f_value of the simplex (with lowest f_value)
        f_values_history(k)=f_values(1);

        % Compute the centroid of all the simplex points minus the worst (with highest f_value)
        ...
        if n == 2
            plot([simplex(1, :), simplex(1, 1)], [simplex(2, :), simplex(2, 1)], '-o', 'LineWidth', 1.5);
        end
